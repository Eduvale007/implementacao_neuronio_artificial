...
def enviar_mensagem():
    ...
    messagebox.showinfo("Pronto", f"Mensagem agendada para {hora}:{minuto:01d}")

# ===== Interface Tkinter =====
root = tk.Tk()
root.title("Bot Clima + WhatsApp")

# Label com Unicode do cérebro piscando
label_cerebro = tk.Label(root, text="🧠", font=("Arial", 50))
label_cerebro.grid(row=0, column=0, columnspan=2, pady=10)

def piscar():
    if label_cerebro.cget("text") == "🧠":
        label_cerebro.config(text="")
    else:
        label_cerebro.config(text="🧠")
